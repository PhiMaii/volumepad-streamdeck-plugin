...
const METER_MODE_OPTIONS = ["ring_fill", "vu_peak_hold", "peak_indicator"];
...
const SETTINGS_CATALOG = [
    ...
    { key: "lowEndstopPosition", label: "Low Endstop Position", type: "number", min: -1, max: 1.0, step: 0.01, defaultValue: DEFAULT_EFFECTIVE_SETTINGS.lowEndstopPosition },
    ...
    { key: "highEndstopPosition", label: "High Endstop Position", type: "number", min: -1, max: 1.0, step: 0.01, defaultValue: DEFAULT_EFFECTIVE_SETTINGS.highEndstopPosition },
...
const settingsCatalogByKey = Object.create(null);
for (const entry of SETTINGS_CATALOG) {
    settingsCatalogByKey[entry.key] = entry;
}
const SETTINGS_CATALOG_BY_KEY = settingsCatalogByKey;
const SETTINGS_CATALOG_KEYS = SETTINGS_CATALOG.map((entry) => entry.key);
...
function isSettingKey(value) {
    return SETTINGS_CATALOG_KEYS.includes(value);
}
...
function coerceSettingValue(entry, rawValue) {
    switch (entry.type) {
        case "bool": {
...
function normalizeChangeVolumeDirection(value) {
    return value === "Decrease" ? "Decrease" : "Increase";
}
function normalizeChangeVolumeStep(value, fallbackStep = DEFAULT_EFFECTIVE_SETTINGS.volumeStepSize) {
...
function normalizeChangeVolumeSettings(settings, fallbackStep = DEFAULT_EFFECTIVE_SETTINGS.volumeStepSize) {
...
function normalizeChangeSettingsSettings(settings) {
...
export { DEFAULT_EFFECTIVE_SETTINGS as D, SETTINGS_CATALOG_BY_KEY as S, normalizeHexColor as a, SETTINGS_CATALOG as b, coerceSettingValue as c, normalizeChangeSettingsSettings as d, isHexColor as i, normalizeChangeVolumeSettings as n, streamDeck as s };
//# sourceMappingURL=validation-z9OLXDe4.js.map
